...
export default function Project({
  ...
  const {scrollYProgress}=  useScroll({
   ...
   const opacityProgess= useTransform(scrollYProgress,[0,1],[0.6,1]);
    ...
    overflow-hidden sm:pr-8 relative sm:h-[20rem] rounded-lg group-even:pl-18  hover:bg-gray-200 transition'>
        <div className=' pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%]
         flex flex-col h-full group-even:ml-[18rem]'>
        <h3 className=' text-2xl font-semibold'>{title}</h3>
        <div className=' mt-2 leading-relaxed text-gray-700'> <a href={url}>{description}</a></div>
